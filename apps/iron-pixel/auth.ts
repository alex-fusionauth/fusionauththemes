...
export const { handlers, auth, signIn, signOut } = NextAuth(async () => {
  if (
    !process.env.AUTH_FUSIONAUTH_TENANT_ID ||
    !process.env.AUTH_FUSIONAUTH_ISSUER ||
    !process.env.AUTH_FUSIONAUTH_CLIENT_ID ||
    !process.env.AUTH_FUSIONAUTH_CLIENT_SECRET
  ) {
    ...
    providers: [
      {
        ...
        // wellKnown: `${process.env.AUTH_FUSIONAUTH_ISSUER}/.well-known/openid-configuration`,
          ...
          conform: async (response: Response) => {
            ...
            const newHeaders = Array.from(response.headers.entries())
              ...
              .reduce(
                (headers, [key, value]) =>
                  (
                    // biome-ignore lint/style/noCommaOperator: <huh>
